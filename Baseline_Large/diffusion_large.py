...
n_epochs = 200
lr = 5e-5
Dataset_dir = 'Dataset/GOPRO_256'
root_dir = 'Baseline_Large'
model_dir = root_dir+'/saved_model'
result_dir = root_dir+'/result'
log_file = root_dir+'/saved_model/training_log.csv'
...
if __name__ == '__main__':
    ...
    net = nn.DataParallel(ClassConditionedUnet(),device_ids=[0,1,2,3]).to(device)
    ...
    for epoch in range(start_epoch+1, n_epochs):
        ...
        if avg_loss<min_loss:
            min_loss = avg_loss
            torch.save(net.state_dict(), model_dir+f'/best.pt')
